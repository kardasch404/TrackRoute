...
export default function TripsPage() {
  ...
  const handleDeleteTrip = async (trip: Trip) => {
    if (!confirm(`Are you sure you want to delete trip ${trip.tripNumber}?`)) {
      return;
    }
  ...
  const handleAssignSuccess = () => {
    showToast('Trip assigned successfully', 'success');
    refetch();
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="text-2xl font-bold text-yellow-600">
            {trips.filter((t: Trip) => t.status === 'DRAFT' || t.status === 'PENDING').length}
          </div>
        </div>
      </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{selectedTrip.origin.address}</p>
                    <p className="text-gray-400">{selectedTrip.origin.city}</p>
                  </div>
                  <div>
                    <span className="text-gray-500">To:</span>
                    <p className="font-medium">{selectedTrip.destination.address}</p>
                    <p className="text-gray-400">{selectedTrip.destination.city}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{selectedTrip.cargo?.type || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Weight:</span>
                    <span className="font-medium">{selectedTrip.cargo?.weight || 0} kg</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Description:</span>
                    <span className="font-medium">{selectedTrip.cargo?.description || 'N/A'}</span>
                  </div>
                </div>
              </div>
            </div>
