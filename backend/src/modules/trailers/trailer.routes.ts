...
router.post('/', rbacMiddleware([Permission.TRAILER_CREATE]), validationMiddleware(createTrailerSchema), trailerController.createTrailer);
...
router.put('/:id', rbacMiddleware([Permission.TRAILER_UPDATE]), validationMiddleware(updateTrailerSchema), trailerController.updateTrailer);
